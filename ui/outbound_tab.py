...
class OutboundTab:
    def __init__(self, parent_frame, app_instance):
        ...
        self.scan_entry = ctk.CTkEntry(scan_frame, font=self.app.font_normal)
        self.scan_entry.grid(row=0, column=0, sticky="ew")
        self.scan_entry.bind("<Return>", self.process_out)
        add_right_click_menu(self.app, self.scan_entry) # === BỔ SUNG MỚI ===
        ...
        self.transport_combo = ctk.CTkComboBox(outbound_frame, values=[], font=self.app.font_normal)
        ...
        self.driver_combo = ctk.CTkComboBox(outbound_frame, values=[], font=self.app.font_normal)
        self.driver_combo.grid(row=3, column=1, padx=PAD_SMALL, pady=PAD_SMALL, sticky="ew")
    ...
    def update_dropdowns(self):
        drivers = [d['name'] for d in self.entity_manager.get_all_active_drivers()]
        transport_vehicles = [v['license_plate'] for v in self.entity_manager.get_all_active_transport_vehicles()]
        
        self.driver_combo.configure(values=drivers)
        self.driver_combo.set("")
        self.transport_combo.configure(values=transport_vehicles)
